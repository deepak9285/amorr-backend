...
const getAllMessagesofChat = asyncHandler(async (req, res) => {
  ...
  if (!selectedChat.participants?.includes(req.user.profileID)) {
    return res
      .status(400)
      .json(new ApiError(400, "User is not a part of this chat"));
  }
  ...
  const textMessages = messages.map((message) => ({
    ...
    sender: message.msg_sender_amorr_id?.username||"unknown user",
    ...
    senderId: message.msg_sender_amorr_id?._id,
    msg_task_status:message.msg_task_status
  }));
  return res
    .status(200)
    .json(new ApiResponse(200, textMessages, "Messages fetched successfully"));
});
...
const getUserConvfromGroup = asyncHandler(async (req, res) => {
  ...
  const messages = await ChatMessage.find({
    msg_conversation_hash: chatId,
    msg_deleted_status: false,
    msg_sender_amorr_id: req.user._id,
  })
    .sort({ msg_timestamp: -1 })
    .populate("msg_sender_amorr_id", "username");
...
const sendMessage = asyncHandler(async (req, res) => {
  ...
  if (!content && !(req.files?.attachments?.length > 0)) {
    return res
      .status(400)
      .json(new ApiError(400, "Message content or attachment is required"));
  }
  ...
  const msgId =
    (await generateHash(`${chatId}-${userId}-${Date.now()}`)) ||
  ...
  const message = await ChatMessage.create({
    ...
    msg_sender_profileId:profileId,
  ...
  chat.participants.forEach(async (participantObjectId) => {
    ...
    await sendNotification(
      req,
      participantObjectId.toString(),
      MessageEventTypes.MESSAGE_SENT,
      receivedMessage
    );
  });
...
const sendMessagetoReply = asyncHandler(async (req, res) => {
  ...
  chat.participants.forEach(async (participantObjectId) => {
    ...
    await sendNotification(
      req,
      participantObjectId.toString(),
      MessageEventTypes.MESSAGE_REPLIED,
      receivedMessage
    );
  });
  console.log("done emit");
...
const deleteMessage = asyncHandler(async (req, res) => {
  ...
  req.user = user;
  // console.log("chatid",chatId, req.user._id);
  const profileId=user.profileID;
 ...
 // console.log("profile", message.msg_sender_profileId?.toString());
