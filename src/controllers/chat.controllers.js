...
const getUserStatus = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, userStatus, "User status fetched successfully"));
});
...
const createOrGetAOneOnOneChat = asyncHandler(async (req, res) => {
    ...
    if (!userProfileID || !receiverProfileID) {
      return res
        .status(401)
        .json(new ApiError(401, "Both sender and receiver profile IDs are required"));
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json(new ApiError(404, "Sender (user) not found by profile ID"));
    }
    req.user = user;
  
    // Fetch receiver by profileID
    const receiver = await User.findOne({ profileID: receiverProfileID });
    if (!receiver) {
      return res
        .status(404)
        .json(new ApiError(404, "Receiver not found by profile ID"));
    }
  
    // Prevent users from chatting with themselves
    if (receiver.profileID === req.user.profileID) {
      return res
        .status(400)
    ...
    // Generate a chat ID based on profile IDs
    const chatId = await generateHash(
      req.user.profileID.toString() + receiver.profileID.toString()
    );
...
const createAGroupChat = asyncHandler(async (req, res) => {
  ...
  if (members.length < 3) {
    return res
      ...
      .json(
        new ApiError(
          400,
          "Group chat must have at least 3 members including the admin."
        )
      );
  }
  ...
  if (existingGroupChat) {
    return res
      .status(200)
      .json(
        new ApiResponse(200, existingGroupChat, "Group chat already exists")
      );
  }
...
const addUserToGroup = asyncHandler(async (req, res) => {
  ...
  if (groupChat.admin.toString() !== userId.toString()) {
    return res
      .status(403)
      .json(new ApiError(403, "Only the admin can add users to the group"));
  }

  if (groupChat.participants.includes(newUserId)) {
    return res
      .status(400)
      .json(new ApiError(400, "User is already a participant in the group"));
  }

  groupChat.participants.push(new mongoose.Types.ObjectId(newUserId));
  await groupChat.save();

  emitSocketEvent(req, newUserId, ChatEventEnum.USER_ADDED_TO_GROUP, {
    groupId: groupChat._id,
  });

  return res
    .status(200)
    .json(new ApiResponse(200, null, "User added to group successfully"));
});
...
const editGroupDetails = asyncHandler(async (req, res) => {
  ...
  if (groupChat.admin.toString() !== userId.toString()) {
    return res
      .status(403)
      .json(new ApiError(403, "Only the admin can edit group details"));
  }
    ...
    if (uniqueParticipants.length < 3) {
      return res
        ...
        .json(
          new ApiError(
            400,
            "Group must have at least 3 members, including the admin"
          )
        );
    }
  ...
  groupChat.participants.forEach((participant) => {
    emitSocketEvent(req, participant.toString(), ChatEventEnum.GROUP_UPDATED, {
      groupId: groupChat._id,
      name: groupChat.name,
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, groupChat, "Group details updated successfully")
    );
});
...
const removeUserFromGroup = asyncHandler(async (req, res) => {
  ...
  if (groupChat.admin.toString() !== userId.toString()) {
    return res
      .status(403)
      .json(new ApiError(403, "Only the admin can remove users"));
  }

  if (!groupChat.participants.includes(userIdToRemove)) {
    return res
      .status(400)
      .json(new ApiError(400, "User is not a participant of this group"));
  }
...
const blockUser = asyncHandler(async (req, res) => {
  ...
  emitSocketEvent(req, userToBlockId, ChatEventEnum.USER_BLOCKED, {
    userId: req.user._id,
  });
...
const checkBlockStatus = asyncHandler(async (req, res) => {
  ...
  const isBlocked =
    userToCheck.userBlocked &&
    userToCheck.whoBlocked?.toString() === req.user._id.toString();
